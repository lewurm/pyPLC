...
start_tcpdump() {
    ...
    OUTPUT_FILE="$1"
    sudo echo "please activate sudo"
    sudo killall tcpdump || true
    sudo tcpdump -i "$INTERFACE" -w "$OUTPUT_FILE" &> /dev/null &
    TCPDUMP_PID=$!
...
logfile=./log/"$date"_evse_tesla_model3_tcpdump.log
tcpdump_logfile=./log/"$date"_evse_tesla_model3_tcpdump.pcap
